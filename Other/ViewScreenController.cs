    ...
    private void Update()
    {
        if (!Disabled)
        {
            ...
            if (TargetEngine.SoundControl.ThisFrameDist > 2000f && !TargetEngine.IsOwner)
                TargetEngine.EffectsControl.Effects();//this is skipped in effectscontroller as an optimization if plane is distant, but the camera can see it, so do it here.
